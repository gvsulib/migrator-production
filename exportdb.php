...
function exportSKUs($exp_limit){
	global $books_table, $new_entries;
	$newline = "\r\n";
	/*
...
function exportdb($exp_limit){
	global $EXP_DELIM, $books_table, $new_entries;
	$newline = "\r\n";
	/*
	 * Dematic needs a text file that line by line has:
	 * carrier_code,cell_id,sku,dedicated
	...
	if(!$new_entries){
		$sql = "SELECT sku, box_code, dedicated, carrier_label, cell_id FROM $books_table WHERE carrier_label != '0'";	//production
	} else {
		$sql = "SELECT sku, box_code, dedicated, carrier_label, cell_id FROM $books_table WHERE exported = '0'
			AND carrier_label != '0'";	//production
	}
	if($exp_limit > 0){
		$sql .= " LIMIT ".$exp_limit;
	}
	...
	mysql_query("UPDATE $books_table SET exported = '1' WHERE exported = '0' AND carrier_label != '0'") or die(mysql_error());
